...
supabase/point_rates.sql            marks the 25 point/circuit rates category 'Point'
supabase/merge_gopinath.sql         one customer, entered twice with a different capital R
supabase/earthing_and_bescom.sql    Earthing template, BESCOM charges, niles in Kg
supabase/import_old_bills.sql       his 2017-2022 Excel bills, generated by tools/gen_import.py
tools/xlsx.py                       reads .xlsx with zipfile + ElementTree, no library
tools/gen_import.py                 those workbooks -> the import SQL
tools/compare.py                    his 2017/2020 point rates against the rate card
...
- **A whole house is priced by points, and that is a third kind of bill.**
  `/documents/new?type=estimate&points=1` lays out every rate with
  `category = 'Point'` — 25 of them, counts blank — so the job is counting
  rooms rather than hunting them out of 273 items. It is the shape his
...
- **The draft key has to vary with the shape of the form, not just the
  document type.** A point sheet and a plain estimate are both
  "new-estimate", so one restored over the other — hence `variant`. And
  "an unsaved draft was restored" must mean he typed something: a
  preloaded row carries a description with no quantity, which announced
  restored work every time he opened a fresh point sheet.
...
- **His old Excel sheets use two pricing models, and they do not
  ...
  together. Repairs and section work are priced as materials plus a
  labour lump, which is what every 2026 bill in the app does. The rate
  card's ₹175 light point is the second kind, so it is not a 68% price
  cut; comparing them as though it were is the easy mistake. What *does*
  compare is the per-foot circuit rates, which appear in both and have
  fallen in nominal terms since 2020.
- **Read a workbook with `tools/xlsx.py`, never by hand.** xlsx is a zip
  of XML and the sheets hold hundreds of lines; `tools/gen_import.py`
  turns them into SQL so nothing is retyped. A line with a description
  and a figure but no quantity is a lump (a BESCOM deposit, a labour
  charge) and must be kept — reading its figure as a rate and dropping it
  for want of a quantity silently lost ₹21,500 of one bill.
